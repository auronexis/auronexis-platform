# Deployment Guide — v0.99

**Date:** 2025-06-23  
**Version:** Auroranexis v0.99.0  
**Sprint:** Phase 5 Sprint 4 — Staging Deployment, Production Validation & Cost Analysis  
...
- [ ] PR opens → Vercel preview URL generated
...
- [ ] `staging.auroranexis.com` → staging project Production env
- [ ] `app.auroranexis.com` → production project Production env
- [ ] `auroranexis.com` → marketing routes (same or separate project)
...
- [ ] Deployment readiness → SSL / HTTPS = green on staging/production

### Caching

- [ ] `/api/health` — no-store (configured in `vercel.json`)
- [ ] Marketing pages — Next.js static/ISR where applicable
- [ ] Authenticated dashboard — dynamic, no CDN cache on private routes
...
- [ ] Diagnostics → Cron infrastructure shows healthy
...
Promote previous Vercel deployment from Deployments tab — see [vercel-deployment.md](./vercel-deployment.md#rollback-strategy).
