...
export function NewsletterSignupForm({ className }: { className?: string }) {
  ...
  if (state.success) {
    return <FormAlert variant="success" className={className}>You are subscribed. Welcome to the Auroranexis updates list.</FormAlert>;
  }
    ...
    <form action={formAction} className={cn("space-y-3", className)}>
      <div className="flex flex-col gap-3 sm:flex-row">
        <input
          ...
          className="min-w-0 flex-1 rounded-lg border border-white/10 bg-white/[0.04] px-3 py-2 text-sm text-white"
        />
