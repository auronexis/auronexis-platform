...
function resolveEventLabel(eventType: string): string {
  ...
  return eventType.replace("risk.", "").replaceAll("_", " ");
}
  ...
  if (events.length === 0) {
    return (
      <p className={cn("text-sm text-muted", className)}>
        No timeline events recorded yet. Activity appears when risks are created or updated.
      </p>
    );
  }
        ...
        <li key={event.id} className="relative flex gap-4 pb-8 last:pb-0">
          ...
          <div className="min-w-0 flex-1 pt-0.5">
            ...
            <p className="mt-2 text-xs text-muted">
              {event.actor?.full_name ?? "System"} · {formatRiskDateTime(event.created_at)}
            </p>
          </div>
        </li>
      ))}
