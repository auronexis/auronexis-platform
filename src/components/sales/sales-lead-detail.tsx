...
import { PIPELINE_STAGES, getLeadSourceLabel } from "@/lib/sales/pipeline-stages";
...
export function SalesLeadDetail({ lead, activities, canManage }: SalesLeadDetailProps) {
      ...
      <section className="aurora-surface p-6">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-muted">{getLeadSourceLabel(lead.lead_source)} · {lead.company_name ?? "No company"}</p>
          </div>
        ...
        {canManage ? (
          <form action={updateAction} className="mt-6 space-y-4">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <label className="block text-sm">
                <span className="mb-1 block font-medium text-foreground">Lead value</span>
              ...
              <label className="block text-sm">
                ...
                <input name="website" defaultValue={lead.website ?? ""} className="w-full rounded-lg border border-border-subtle bg-surface-1 px-3 py-2 text-sm" />
              </label>
              ...
              <label className="block text-sm">
                ...
                <input name="location" defaultValue={lead.location ?? ""} className="w-full rounded-lg border border-border-subtle bg-surface-1 px-3 py-2 text-sm" />
              </label>
              <label className="block text-sm">
          ...
          <div className="mt-6 space-y-3 text-sm text-muted">
            <p><strong className="text-foreground">Stage:</strong> {lead.pipeline_stage}</p>
