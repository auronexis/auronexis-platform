import type { ConnectorModuleConfig } from "@/lib/connectors/shared/module-config";
import { GOOGLE_CONNECTOR_CONFIG } from "@/lib/connectors/definitions";

export type GoogleWebhookEvent = {
  id: string;
  type: string;
  payload: Record<string, unknown>;
};

export function parseGoogleWebhook(
  payload: Record<string, unknown>,
): GoogleWebhookEvent {
  return {
    id: String(payload.id ?? payload.event_id ?? "unknown"),
    type: String(payload.type ?? payload.event ?? "unknown"),
    payload,
  };
}

export function validateGoogleWebhookSignature(
  _secret: string,
  _body: string,
  _signature: string | null,
): boolean {
  return Boolean(_signature);
}

export const WEBHOOK_CONFIG: ConnectorModuleConfig = GOOGLE_CONNECTOR_CONFIG;
