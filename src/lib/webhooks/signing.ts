...
export function buildWebhookSignatureHeaders(input: {
  ...
  const signature = signWebhookPayload(input.secret, input.payload, Number(timestamp));
  ...
  return {
    ...
    headers: {
      ...
      "x-auroranexis-event": input.eventType,
      "x-auroranexis-delivery-id": input.deliveryId,
    },
  };
}
