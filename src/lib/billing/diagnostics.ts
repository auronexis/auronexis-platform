...
import { checkMollieApiConfigHealth } from "@/lib/diagnostics/platform-health";
...
export async function getBillingDiagnosticsSnapshot(
  ...
  const mollieHealth = checkMollieApiConfigHealth();
  ...
  return {
    ...
    stripeConnected: mollieHealth.ok, // legacy field name — Mollie config health
