...
import type { ApiRequestLog, WebhookDelivery, WebhookEndpoint } from "@/types/database";
...
export async function getApiDashboardSnapshot(session: SessionContext): Promise<ApiDashboardSnapshot> {
  ...
  const [{ data: logs }, { data: endpoints }, { data: deliveries }] = await Promise.all([
    ...
    supabase.from("webhook_endpoints").select("*").eq("organization_id", session.organization.id),
    supabase
  ...
  const endpointRows = (endpoints ?? []) as WebhookEndpoint[];
...
function mapEndpoint(row: WebhookEndpoint): ApiWebhookEndpointView {
  return {
    id: row.id,
...
function mapDelivery(row: WebhookDelivery): ApiWebhookDeliveryView {
  return {
    id: row.id,
    ...
    errorMessage: row.response_body,
