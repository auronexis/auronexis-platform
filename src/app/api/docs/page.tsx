...
export const metadata: Metadata = {
  title: "API Reference",
  description: "Auroranexis REST API authentication, endpoints, and OpenAPI specification.",
};
    ...
    <div className="min-h-screen bg-background text-foreground">
      <header className="border-b border-border/70 bg-surface/40">
        <div className="mx-auto flex max-w-5xl flex-wrap items-center justify-between gap-4 px-6 py-5">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.12em] text-muted">API Reference</p>
            <h1 className="mt-1 text-2xl font-semibold tracking-tight">{spec.info.title}</h1>
              ...
              <tbody>
                {endpoints.map((endpoint) => (
                  <tr key={`${endpoint.method}-${endpoint.path}`} className="border-b border-border/60 last:border-0">
                    ...
                    <td className="px-3 py-3 align-top font-mono text-foreground">{endpoint.path}</td>
                    <td className="px-3 py-3 align-top text-muted">{endpoint.summary}</td>
                  </tr>
                ))}
              </tbody>
            </table>
